...
# Helper function for easier usage in scripts
def add_crypto_features(df: pd.DataFrame, feature_set: str = 'standard') -> Tuple[pd.DataFrame, List[str]]:
    """
    Add cryptocurrency features and return selected feature columns.
    
    Args:
        df: DataFrame with at least 'price' and 'volume' columns
        feature_set: Which set of features to select ('minimal', 'standard', or 'full')
        
    Returns:
        Tuple of (DataFrame with features added, list of selected feature columns)
    """
    engineer = CryptoFeatureEngineer()
    result_df = engineer.add_all_features(df)
    selected_features = engineer.get_selected_features(result_df, feature_set)
